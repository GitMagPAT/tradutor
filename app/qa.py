...
# Variações corrompidas já observadas em saída (ex.: "ZXENTX0001ZXQ", "ZQENT...",
# "ZXXQENT...", etc.). Mantemos o padrão relativamente estrito (precisa ter
# ENT|GLOS e algum dígito) para evitar falsos positivos.
...
def run_qa_scan(workdir: Path, out_pdf: Path, cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Rodar um scanner de qualidade no fim do pipeline.

    Objetivos (baixo/médio risco):
    - Detectar vazamento de placeholders (ZXQENT/ZXQGLOS/ZXQSEGBOUNDARY, etc.).
    - Sinalizar páginas com taxa muito alta de blocos "unchanged" apesar de
      terem bastante texto nativo.
    - Gerar artefatos em `work/qa_report.json` e `work/qa_report.txt`.

    Por padrão, falha o run se detectar "ZXQ" no PDF final, pois isso é sempre
    um bug (placeholders nunca podem vazar).
    """
    ...
    for lf in log_files:
        try:
            ...
            # Não falha por um log isolado; apenas ignora.
            continue
    ...
    try:
        ...
        for i in range(len(doc)):
            ...
            # Qualquer resíduo de token interno é bug.
            if _ZXQ_RE.search(txt) or _ZXQ_FUZZY_RE.search(txt):
                # Pega um snippet curto para facilitar debug
                m = re.search(
                    r"(?i)(.{0,30}(?:ZXQ|Z[XQ]{0,4}(?:ENT|GLOS)\w*\d).{0,30})",
                    txt,
                )
                snippet = (m.group(1) if m else "ZXQ")
    ...
    try:
        (Path(workdir) / "qa_report.json").write_text(
        ...
        if issues:
            ...
            for it in issues[:80]:
                if it.get("type") == "zxq_leak":
                    lines.append(
                        f"- [ERROR] zxq_leak: {it.get('count')} página(s). Ex.: {it.get('pages', [])[:3]}"
                    )
        ...
        # QA nunca deve quebrar o run por falha ao salvar o relatório
        pass

    # 4) Gate de falha (placeholders)
